...
class TestTimeseries:
    ...
    def test_400_freq_mhz_non_numeric(self, flask_client):
        _create_band()
        r = flask_client.get(f"/api/bands/{BAND_ID}/timeseries?freq_mhz=abc")
        assert r.status_code == 400
        assert "error" in json.loads(r.data)


# ── input validation — shared filter params ───────────────────────────────────

class TestFilterValidation:
    """Bad query-param values must return 400, never 500."""

    def test_heatmap_400_freq_min_non_numeric(self, flask_client):
        _create_band()
        r = flask_client.get(f"/api/bands/{BAND_ID}/heatmap?freq_min=abc")
        assert r.status_code == 400
        assert "freq_min" in json.loads(r.data)["error"]

    def test_heatmap_400_freq_max_non_numeric(self, flask_client):
        _create_band()
        r = flask_client.get(f"/api/bands/{BAND_ID}/heatmap?freq_max=xyz")
        assert r.status_code == 400

    def test_heatmap_400_power_min_non_numeric(self, flask_client):
        _create_band()
        r = flask_client.get(f"/api/bands/{BAND_ID}/heatmap?power_min=strong")
        assert r.status_code == 400
        assert "power_min" in json.loads(r.data)["error"]

    def test_spectrum_400_freq_min_non_numeric(self, flask_client):
        _create_band()
        r = flask_client.get(f"/api/bands/{BAND_ID}/spectrum?freq_min=bad")
        assert r.status_code == 400

    def test_activity_400_threshold_non_numeric(self, flask_client):
        _create_band()
        r = flask_client.get(f"/api/bands/{BAND_ID}/activity?threshold=strong")
        assert r.status_code == 400
        assert "threshold" in json.loads(r.data)["error"]

    def test_noise_floor_400_invalid_granularity(self, flask_client):
        _create_band()
        r = flask_client.get(f"/api/bands/{BAND_ID}/noise-floor?granularity=1week")
        assert r.status_code == 400
        assert "granularity" in json.loads(r.data)["error"]

    def test_noise_floor_valid_granularities_accepted(self, flask_client):
        """All documented granularity values must be accepted (may 404 with no data)."""
        _create_band()
        for gran in ("15m", "30m", "1h", "6h", "1d"):
            r = flask_client.get(f"/api/bands/{BAND_ID}/noise-floor?granularity={gran}")
            assert r.status_code in (200, 404), f"granularity={gran!r} gave unexpected {r.status_code}"
